		...
		with tf.name_scope(self.name):
			...
			with tf.name_scope('stacks'):
				with tf.name_scope('hourglass.1'):
					...
					out[0] = conv2d(ll[0],self.outDim,1,1,'VALID','out') # 64x64x[16*num_points] -> reshape
				...
				for i in range(1, self.nbStack-1):
					with tf.name_scope('hourglass.' + str(i+1)):
						...
						out[i] = conv2d(ll[i],self.outDim,1,1,'VALID','out') # 64x64x[16*num_points] -> reshape
				...
				with tf.name_scope('hourglass.' + str(self.nbStack)):
					...
					out[self.nbStack-1] = conv2d(drop[self.nbStack-1],self.outDim,1,1,'VALID', 'out') # 64x64x[16*num_points] -> reshape
			self.out = tf.stack(out, name = 'output')
			self.features = tf.stack(self.features, name='features')
		
	def hourglass(self, inputs, n, numOut, name = 'hourglass', is_training=True):
