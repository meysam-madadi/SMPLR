...
'''
Adapted in Oct. 2018 by Hugo Bertiche
'''

import tensorflow as tf
import numpy as np
from Models.StackedHourglass.layers import conv2d, convBnrelu, residual
	...
	def _create(self, inputs):
		with tf.name_scope(self.name):
			with tf.name_scope('preprocessing'):
				...
				res_1 = residual(conv_1, 128, is_training=self.train)
				pool_1 = tf.contrib.layers.max_pool2d(res_1, [2,2], [2,2], padding= 'VALID')
				res_2 = residual(pool_1, 128, is_training=self.train)
				res_3 = residual(res_2, self.nFeat, is_training=self.train)
			# Supervision Table
			...
			with tf.name_scope('stacks'):
				with tf.name_scope('hourglass.1'):
					...
					ll[0] = convBnrelu(hg[0], self.nFeat, name= 'conv_1', is_training=self.train)
					ll_[0] = conv2d(ll[0],self.nFeat,1,1,'VALID','ll')
					drop[0] = tf.layers.dropout(ll_[0], rate = 0.1, training = self.train)
					out[0] = conv2d(ll[0],(self.outDim + self.outBones) * self.voxelDepth,1,1,'VALID','out') # 64x64x[16*num_points] -> reshape
				...
				for i in range(1, self.nbStack-1):
					with tf.name_scope('hourglass.' + str(i+1)):
						...
						ll[i] = convBnrelu(hg[i], self.nFeat, name='conv_1', is_training=self.train)
						ll_[i] = conv2d(ll[i],self.nFeat,1,1,'VALID','ll')
						drop[i] = tf.layers.dropout(ll_[i],rate=0.1, training = self.train)
						out[i] = conv2d(ll[i],(self.outDim + self.outBones) * self.voxelDepth,1,1,'VALID','out') # 64x64x[16*num_points] -> reshape
				...
				with tf.name_scope('hourglass.' + str(self.nbStack)):
					...
					ll[self.nbStack-1] = convBnrelu(hg[self.nbStack - 1], self.nFeat, name='conv_1', is_training=self.train)
					drop[self.nbStack-1] = tf.layers.dropout(ll[self.nbStack-1], rate=0.1, training = self.train)
					out[self.nbStack-1] = conv2d(drop[self.nbStack-1],(self.outDim + self.outBones) * self.voxelDepth,1,1,'VALID', 'out') # 64x64x[16*num_points] -> reshape
			self.features = tf.stack(self.features, name='features')
			self.toVoxel(out)
		
	def hourglass(self, inputs, n, numOut, name = 'hourglass'):
		with tf.name_scope(name):
			up_1 = residual(inputs, numOut, name = 'up1', is_training=self.train)
			low_ = tf.contrib.layers.max_pool2d(inputs, [2,2],[2,2], 'VALID')
			low_1 = residual(low_, numOut, name = 'low1', is_training=self.train)
			if n > 0:
				low_2 = self.hourglass(low_1, n-1, numOut, name='low2')
			else:
				low_2 = residual(low_1, numOut, name='low2', is_training=self.train)
			low_3 = residual(low_2, numOut, name = 'low3', is_training=self.train)
			
			# Keep track of 4x4x256 tensors of each stack
			_shape = low_3.get_shape()
			if _shape[1] == 4:
				self.features.append(low_3)
				
			up_2 = tf.image.resize_nearest_neighbor(low_3, tf.shape(low_3)[1:3]*2, name= 'upsampling')
			return tf.add(up_1, up_2)
		
	def toVoxel(self, out):
		self.out = []
		for o in out:
			self.out.append(tf.reshape(o, (-1, 64, 64, self.voxelDepth, self.outDim + self.outBones)))
		self.out = tf.stack(self.out, name = 'output')