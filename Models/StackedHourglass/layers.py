...
def convBnrelu(inputs, filters, kernel_size = 1, strides = 1, name = None, is_training=True):
	"""
	...
	with tf.name_scope(name):
		...
		norm = tf.contrib.layers.batch_norm(conv, 0.9, epsilon=1e-5, activation_fn = tf.nn.relu, is_training=is_training)
		return norm
	
def convBlock(inputs, numOut, name = 'convBlock', is_training=True):
	"""
	...
	if isinstance(is_training, bool):
		print("WARNING: is_training NOT SET - " + name)
	with tf.name_scope(name):
		norm_1 = tf.contrib.layers.batch_norm(inputs, 0.9, epsilon=1e-5, activation_fn = tf.nn.relu, is_training=is_training)
		conv_1 = conv2d(norm_1, int(numOut/2), kernel_size=1, strides=1, pad = 'VALID')
		norm_2 = tf.contrib.layers.batch_norm(conv_1, 0.9, epsilon=1e-5, activation_fn = tf.nn.relu, is_training=is_training)
		pad = tf.pad(norm_2, np.array([[0,0],[1,1],[1,1],[0,0]]))
		conv_2 = conv2d(pad, int(numOut/2), kernel_size=3, strides=1, pad = 'VALID')
		norm_3 = tf.contrib.layers.batch_norm(conv_2, 0.9, epsilon=1e-5, activation_fn = tf.nn.relu, is_training=is_training)
...
def residual(inputs, numOut, name = 'residual', is_training=True):
	# DIMENSION CONSERVED
	if isinstance(is_training, bool):
		print("WARNING: is_training NOT SET - " + name)
	with tf.name_scope(name):
		convb = convBlock(inputs, numOut, is_training=is_training)
